...
export default function CameraCard() {
  ...
  return (
    <div className="w-full max-w-sm bg-white rounded-2xl shadow-lg p-4 mx-auto">
      <h1 className="text-xl font-bold mb-2 text-center">Scan QR Code</h1>
      
      <p className="text-gray-600 text-xs mb-1 text-center">
        Position QR code in the square frame
      </p>
      
      <p className="text-green-600 text-[10px] mb-2 text-center font-semibold">
        ✅ Using html5-qrcode - TESTED & WORKING!
      </p>

      {error && (
        <div className="mb-2 p-2 bg-red-100 border border-red-400 text-red-700 rounded-lg text-xs">
          {error}
        </div>
      )}

      {success && (
        <div className="mb-2 p-2 bg-green-100 border border-green-400 text-green-700 rounded-lg text-xs font-semibold flex items-center gap-2">
      ...
      <div className="mb-2 w-full relative" style={{ minHeight: scanning ? 'auto' : '0' }}>
        {/* Placeholder - shown when NOT scanning */}
        <div 
          className="w-full rounded-lg flex flex-col items-center justify-center gap-1.5 border-2 border-dashed border-gray-300 bg-gradient-to-br from-gray-100 to-gray-200 p-3"
          style={{ 
        ...
        >
          ...
          <p className="text-gray-500 text-xs font-medium">Camera preview</p>
          <p className="text-gray-400 text-[10px]">Click button below to start</p>
        </div>
      ...
      {scanned && (
        <div className="mb-2 flex justify-center">
          <div className="bg-green-500 text-white rounded-full p-2.5 animate-bounce">
      ...
      <div className="flex flex-col gap-2">
        {!scanning ? (
          <button
            onClick={startCamera}
            disabled={processing}
            className="w-full py-2.5 bg-red-400 hover:bg-red-500 disabled:bg-gray-300 text-white rounded-lg text-sm font-semibold transition-colors"
          >
          ...
          <button
            onClick={stopCamera}
            disabled={processing}
            className="w-full py-2.5 bg-gray-600 hover:bg-gray-700 disabled:bg-gray-300 text-white rounded-lg text-sm font-semibold transition-colors"
          >
            ⏹️ Stop Camera
          </button>
        )}
        ...
        <button
          ...
          disabled={processing}
          className="w-full py-2.5 border-2 border-gray-300 hover:bg-gray-100 disabled:opacity-50 text-gray-700 rounded-lg text-sm font-semibold transition-colors"
        >
          Cancel
        </button>
      </div>
